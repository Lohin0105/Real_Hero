// backend/scripts/generate_translations_gemini.mjs
...
async function translateText(text, targetLanguage) {
    try {
        ...
        const prompt = `Translate the following English text to ${languages[targetLanguage]}. Only return the translated text, nothing else. Do not add quotes or explanations.
...
async function main() {
    console.log('🚀 Starting translation generation with Gemini API...\n');

    if (!process.env.GEMINI_TRANSLATION_API_KEY) {
        console.error('❌ ERROR: GEMINI_TRANSLATION_API_KEY not found in .env file');
        process.exit(1);
    }
